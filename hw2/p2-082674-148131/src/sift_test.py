...
img_orig = cv2.imread(dir_path+'/../input/input-p2-3-1-0.jpg', 1)
if img_orig is None:
...
img_out = img_orig
cv2.drawKeypoints(img, sift.get_keypoints(), img_out, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
cv2.imwrite(output+str(counter)+'.png', img_out)
counter = counter+1
sift.find_descriptors()
sift1_desc = sift.get_descriptors()

img_orig = cv2.imread(dir_path+'/../input/input-p2-3-1-1.jpg', 1)
if img_orig is None:
	print("Image not found.")
	sys.exit()
img = cv2.cvtColor(img_orig	, cv2.COLOR_BGR2GRAY)
