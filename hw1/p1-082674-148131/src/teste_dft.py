...
blend_mask = np.concatenate((blend_mask, np.zeros((height, length-length//2, 1))))

imgleft = np.multiply(imgleft, blend_mask)
imgright = np.multiply(imgright, 1 - blend_mask)
...
fourier_result = fourier.blend(dft_shift_l, dft_shift_r, blend_mask)
