...
def blend(dft_shift_1, dft_shift_2, mask):
...
#	dft_shift_2 = np.fft.fftshift(dft_2)
	...
	phase_spectrum_1 = 40*np.log(cv2.phase(dft_shift_1[:, :, 0], dft_shift_1[:, :, 1], True))
	magnitude_spectrum_2 = 20*np.log(cv2.magnitude(dft_shift_2[:,:,0],dft_shift_2[:,:,1]))
	#magnitude_spectrum = cv2.magnitude(dft_shift[:,:,0],dft_shift[:,:,1])
	phase_spectrum_2 = 40*np.log(cv2.phase(dft_shift_2[:, :, 0], dft_shift_2[:, :, 1], True))
...
#	img_blend = img1_freq + img2_freq
	...
	f_ishift = np.fft.ifftshift(pha_)
	img_back2 = cv2.idft(f_ishift, flags=cv2.DFT_SCALE)
